...
//		[leftAligmentStyle setParagraphStyle:[NSParagraphStyle defaultParagraphStyle]];
		...
		if ((nil != leftCaption) && (NO == [leftCaption isEqualToString:@""])) {
			...
			[sAttr drawAtPoint:CGPointMake(bBounds.origin.x + widthSpace + 0.25 * [barcode firstBar],yPos + 20.0)];
			[sAttr release];
		}
		...
		if ((nil != caption) && (NO == [caption isEqualToString:@""])) {
			...
			if (((nil != leftCaption) && (NO == [leftCaption isEqualToString:@""])) || (1 < [captions count])) {
				...
				for (cCount = 0; cCount < [captions count]; cCount++) {
					...
					[sAttr drawAtPoint:CGPointMake(bBounds.origin.x + widthSpace + kerning * 3 + [barcode firstBar] + (float)(cCount + 1) * [[barcode  initiator] length] * barWidth + (float)cCount * container,yPos+5)];
					[sAttr release];
				}
			}
			else {
				...
				[sAttr drawAtPoint:CGPointMake(bBounds.origin.x + widthSpace + kerning,yPos)];
				[sAttr release];
			}
		}
